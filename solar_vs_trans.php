...
$meterIds = [
    ...
    "G1_U14", "G1_U15", "G1_U16", "G2_U2", "G2_U3", "G2_U4", "G2_U7", "G2_U8", "G2_U9", "G2_U10", "G2_U11",
...
$allGensetKeys = ["G1_U16_ACTIVE_ENERGY_IMPORT_KWH"]; // Added all_genset key
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['start_date']) && isset($_GET['end_date'])) {
    ...
    try {
        ...
        $totalConsumption = [
            "Solars" => 0,
            "Transformers" => 0,
            "All_Genset" => 0 // Added all_genset group
        ];
        ...
        foreach ($dailyConsumption as $consumption) {
            foreach ($consumption as $key => $value) {
                ...
                } elseif (in_array($key, $allGensetKeys)) { // Aggregate all_genset consumption
