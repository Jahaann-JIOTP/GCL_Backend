            ...
            foreach ($meterIds as $meterId) {
                foreach ($suffixes as $suffix) {
                    ...
                    if (isset($firstValuesByDay[$nextDate][$key])) {
                        ...
                        $dailyConsumption[$currentDate][$meterNameMapping[$meterId]] = 
                          ($dailyConsumption[$currentDate][$meterNameMapping[$meterId]] ?? 0) + ($nextValue - $currentValue);
                    }
                }
            }
        }
        ...
        foreach ($meterIds as $meterId) {
            foreach ($suffixes as $suffix) {
                ...
                if (isset($firstValuesByDay[$lastDate][$key]) && isset($lastValuesByDay[$lastDate][$key])) {
                    ...
                    $dailyConsumption[$lastDate][$meterNameMapping[$meterId]] = 
                      ($dailyConsumption[$lastDate][$meterNameMapping[$meterId]] ?? 0) + ($lastValue - $firstValue);
                }
            }
        }
