    ...
    try {
        ...
        if ($timePeriod === "This Week over Last Week") {
            ...
            if (!function_exists('groupIntoWeeks')) {
                function groupIntoWeeks($firstValuesByDay, $lastValuesByDay, $monthStart, $monthEnd, $latestAvailableValues, $includeCurrentWeek = false)
                {
                    $weeks = [];
                    $dayCounter = 0;
                    $weekData = [];
                    ...
                    // Ensure exactly 5 weeks are present
                    for ($i = 0; $i < 5; $i++) {
                        $totalConsumption = array_sum($weekData[$i] ?? []);
                        $weeks[] = [
                            'week' => "Week " . ($i + 1),
                            'value' => $totalConsumption > 0 ? $totalConsumption : 0
                        ];
                    }
            ...
            // Define date ranges for last and current months
            ...
            // Generate results for "This Month over Last Month"
            $result = [
                ...
                "This Month Weeks" => groupIntoWeeks(
                    ...
                    $thisMonthEnd,
                    $latestAvailableValues,
                    true // Include the current partial week
                )
            ];
        }
         elseif ($timePeriod === "Today over Yesterday") {
            ...
            foreach ([$yesterday, $today] as $day) {
                foreach ($allHours as $hour) {
                    ...
                    // If no data exists for this hour, set default 0 values
